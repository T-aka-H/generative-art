...
var PRESET_COLORS = [];
var userHueIdx = [0, 2, 4, 8, 10]; // 5 skaters: Red, Yellow, Green, Blue, Magenta
var userColors = [];
...
function setup() {
  ...
  for (var i = 0; i < 5; i++) userColors.push(PRESET_COLORS[userHueIdx[i]]);
...
function initSkaters() {
  ...
  for (var i = 0; i < NUM_SKATERS; i++) {
    var positions = [0.25, 0.38, 0.5, 0.62, 0.75];
    var bs = 2.5 + random(0, 2.0);
    var slot = i;
    var col = userColors[slot];
    skaters.push({
...
function updateUIRects() {
  ...
  var gap = 12;
  var rightMargin = 14;
  var totalHeight = uiSize * 6 + gap * 5; // 5 color + 1 eraser
...
// ============================================
// Physics — square trajectory (90° turns only)
// ============================================
function updateSkater(idx) {
  ...
  // Square trajectory: straight + 90° turns
  if (s.turning > 0) {
    ...
    if (s.segDist >= s.segLen) {
      s.segDist = 0;
      s.segLen = 35 + noise(s.seed + 300, t * 0.05) * 65;
      s.turnTarget = HALF_PI + (noise(s.seed + 400, t * 0.1) - 0.5) * 0.5;
  ...
  // Edge steering (hard threshold at 0.85)
  ...
  if (edge > 0.85) {
    ...
    s.turning = 0;
    s.segDist = 0;
    s.segLen = 40 + random(60);
  }
}

// ============================================
// Collisions — bounce + color change
// ============================================
  ...
  for (var i = 0; i < skaters.length; i++) {
    ...
    for (var j = i + 1; j < skaters.length; j++) {
      ...
      if (d < COLLISION_DIST && (!collisionCooldowns[key] || now - collisionCooldowns[key] > COLLISION_COOLDOWN)) {
        ...
        // Sparks (8 spinning square particles, pre-collision colors)
        for (var k = 0; k < 8; k++) {
          var src = k < 4 ? a : b;
...
function draw() {
  ...
  // Skater heads — multi-layer square glow
  for (var i = 0; i < skaters.length; i++) {
    var s = skaters[i];
    if (!s.active || s.fadeIn < 0.01) continue;
    var fi = s.fadeIn;
    ...
    var heads = [
      { r: 30 * sc, a: 0.025 * pulse * fi },
      { r: 16 * sc, a: 0.07 * pulse * fi },
      { r: 8 * sc, a: 0.16 * pulse * fi },
      { r: 4 * sc, a: 0.4 * pulse * fi },
      { r: 2 * sc, a: 0.8 * pulse * fi }
    ];
    for (var hi = 0; hi < heads.length; hi++) {
      ctx.fillStyle = 'rgba(' + colStr + ',' + heads[hi].a + ')';
      drawSquare(ctx, s.x, s.y, heads[hi].r, s.angle + Math.sin(t * 0.3 + s.spinOffset) * 0.15);
      ctx.fill();
    }
    // White core
    ctx.fillStyle = 'rgba(255,240,255,' + (pulse * fi) + ')';
    drawSquare(ctx, s.x, s.y, 1.8 * sc, s.angle);
    ctx.fill();
  }

  // Sparks — spinning squares
  for (var si = 0; si < sparks.length; si++) {
    var sp = sparks[si];
    ...
    drawSquare(ctx, sp.x, sp.y, 3.5 * fade, sp.angle);
    ctx.fill();
    ctx.fillStyle = 'rgba(255,255,255,' + (fade * 0.35) + ')';
    drawSquare(ctx, sp.x, sp.y, 1.2 * fade, sp.angle);
    ctx.fill();
  }
...
function drawColorUI(ctx) {
  ...
  for (var i = 0; i < colorUIRects.length; i++) {
    ...
    // Check if this skater is active
...
function scatterSkaters(tapX, tapY) {
  for (var i = 0; i < skaters.length; i++) {
    ...
    s.turning = 0; s.segDist = 0; s.segLen = 40 + random(60);
  }
  // Ripple at tap point
